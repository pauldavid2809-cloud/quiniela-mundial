...
export default function QuinielaClient({ phases, matches, predictions, userId }: Props) {
  ...
  const handlePredict = async (matchId: number, prediction: 'home' | 'draw' | 'away') => {
    ...
    if (error) {
      console.error('Error predicción:', error.message)
  ...
  return (
    <div className="animate-fade-in">
      ...
      {errorMsg && (
        <div className="mb-4 p-3 rounded-lg bg-red-900/30 border border-red-500/40 text-red-300 text-sm animate-fade-in">
          ⚠️ {errorMsg}
        </div>
      )}
      ...
      {!currentPhase?.is_unlocked ? (
        <div className="glass-card p-12 text-center">
          ...
          <p className="text-white/40">Esta fase se desbloqueará cuando se confirmen todos los equipos participantes.</p>
        </div>
          ...
          {Object.entries(grouped).sort().map(([groupName, groupMatches], idx) => (
            <div key={groupName} className={`animate-slide-up stagger-${Math.min(idx + 1, 5)}`}>
              <div className="flex items-center gap-3 mb-3">
                ...
                <span className="font-display text-lg text-gold-500 tracking-widest px-3">{groupName}</span>
